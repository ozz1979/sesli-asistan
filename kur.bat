@echo off
chcp 65001 >nul 2>&1
title Sesli AI Asistan v7.1 - Kurulum

echo ================================================
echo    SESLI AI ASISTAN v7.1 - KURULUM
echo ================================================
echo.
...
if "%KEY_STATUS%"=="KEY_MISSING" (
    ...
    echo.
    set /p API_KEY="API anahtarini yapistiriniz: "
    if defined API_KEY (
        venv\Scripts\python.exe -c "import json,sys; c=json.load(open('config.json','r',encoding='utf-8')); c['gemini_api_key']=sys.argv[1]; json.dump(c,open('config.json','w',encoding='utf-8'),ensure_ascii=False,indent=4)" "%API_KEY%" 2>nul
...
if /i "%OTO_BASLAT%"=="E" (
    ...
    echo [OK] Otomatik baslatma atlandi.
)
echo.
